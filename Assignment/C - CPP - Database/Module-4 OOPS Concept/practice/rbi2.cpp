...
int main() {
    ...
    ICICI i;
    PNB p;
    Bank* currentBank = nullptr; // Pointer to manage the current bank instance
    ...
    do {
        ...
        switch (choice) {
            case 1:
                currentBank = &s;
                currentBank->set_interest_rate();
                currentBank->set_minimum_balance();
                currentBank->set_maximum_withdraw();
                s.sbi_details();
                cout << "\n-> Create account in SBI: " << endl;
                currentBank->input_details();
                break;
        ...
        if (currentBank) {
            do {
                cout << "\nEnter 1 to Deposit Money \t 2 to Withdraw Money \t 3 to Display Account Info \t 4 to Bank T&C \t 0 to Exit" << endl;
                cout << "Enter your choice: ";
                cin >> ch;

                switch (ch) {
                    case 1:
                        currentBank->deposit_money();
                        break;

                    case 2:
                        currentBank->withdraw_money();
                        break;

                    case 3:
                        currentBank->display_info();
                        break;

                    case 4:
                        if (choice == 1) s.sbi_details();
                        if (choice == 2) i.icici_details();
                        if (choice == 3) p.pnb_details();
                        break;
